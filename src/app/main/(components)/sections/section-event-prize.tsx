  ...
  },
  {
    ...
    description: () => (
      <DotList className='gap-0'>
        ...
        <DotItem className='text-[1.6rem] leading-[2.6rem] [--line-height:2.6rem]'>독창적으로 제작이 되었는지</DotItem>
      </DotList>
    ),
    score: 30
  },
            ...
            <div className='flex gap-[3.2rem] overflow-hidden'>
              {PRIZE_LIST.map(prize => (
                <div className='flex flex-1 flex-col items-center overflow-hidden' key={prize.title}>
                  ...
                  <div className='pt-[1.6rem] text-center flex flex-col gap-[.4rem]'>
                    <p className='text-[2rem] leading-[3.2rem] font-bold'>{prize.prizeName}</p>
                    <p className='text-[1.6rem] leading-[2.6rem] text-sub-text'>{prize.winDescription}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div>
                {JUDGE_CRITERIA.map(criteria => (
                  <div key={criteria.title} className='flex'>
                    <div className='flex-[0_0_30rem] flex items-center justify-center p-[2.4rem] bg-[#f8f8f8] border-r border-b border-border'>
                      <p className='text-[1.6rem] leading-[2.6rem] font-bold text-center'>{criteria.title}</p>
                    </div>
                    <div className='flex-1 p-[2.4rem] border-r border-b border-border'>{criteria.description()}</div>
                    <div className='flex-[0_0_12rem] flex items-center justify-center p-[2.4rem] border-b border-border'>
