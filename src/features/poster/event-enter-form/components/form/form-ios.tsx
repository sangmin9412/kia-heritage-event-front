...
export const EventEnterFormIOS = () => {
  ...
  useEffect(() => {
    if (formState.isParticipated === false && !formState.error) {
      router.push(ROUTES.CREATE_FORM.link);
    }
  }, [formState.isParticipated, formState.error, router]);
  ...
  return (
    <Form {...form}>
      <form
        ...
        className='flex flex-col desktop:ml-[-1.2rem] desktop:pl-[1.2rem] desktop:mr-[-3.6rem] desktop:pr-[3.6rem] mr-[-1rem] pr-[1rem]'
      >
        <div>
        ...
        <div className='desktop:py-[2.4rem] py-[1.6rem] sticky bottom-0 bg-white'>
